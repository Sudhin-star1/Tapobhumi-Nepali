...
export default function AdminTreks() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`${API_URL}/api/treks${editing ? `/${editing._id}` : ""}`, {
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["treks"] });
      toast({ title: editing ? "Trek updated" : "Trek created" });
      setOpen(false);
      setEditing(null);
      setName("");
      setDescription("");
  ...
  const del = useMutation({
    mutationFn: (id: string) => apiFetch(`/api/treks/${id}`, { method: "DELETE" }),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["treks"] });
  ...
  const rows = treks.data ?? [];

  const removeImage = useMutation({
    mutationFn: async (url: string) => {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(`${API_URL}/api/treks/${editing?._id}/images/remove`, {
        method: "PATCH",
        headers: {
  ...
  const reorderImages = useMutation({
    mutationFn: async (images: string[]) => {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(`${API_URL}/api/treks/${editing?._id}/images/reorder`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold">Treks</h1>
          <p className="text-sm text-muted-foreground">Manage trekking list shown on the site.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editing ? "Edit Trek" : "Add Trek"}</DialogTitle>
            </DialogHeader>
